    ...
    <div className="main-page">
      <div className="product-grid">
        {products.map((product, index) => (
          <Link to={`/category/${encodeURIComponent(product.name)}`} key={index} className="product-box">
            <img src={product.image || '/images/default.png'} alt={product.name} className="product-image" />
            <h3>{product.name}</h3>
            <p>{product.des}</p>
          </Link>
        ))}
